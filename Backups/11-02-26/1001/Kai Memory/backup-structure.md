...
```
~/Documents/Kai/Repos/
├── mind-electron/            # Current Electron app (ACTIVE)
│   ├── app/
...
│   │       └── hhmm/
│   └── ...
├── mind/                     # Old Tauri app (DEPRECATED)
│   ├── app/                 # Legacy Tauri files
│   ├── demo/
│   └── Backups/
├── flow/
│   ├── app/
...
**IMPORTANT:** The active MIND app is `mind-electron/`, NOT `mind/`. 
The `mind/` folder contains old Tauri code. Always work in `mind-electron/`.
